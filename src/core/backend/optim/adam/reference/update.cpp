...
float checked_float(double value, const char* description) {
    ...
    if (!std::isfinite(value) || !std::isfinite(result)) {
        throw std::overflow_error(std::string("Adam produced a non-finite ") +
                                  description);
    }
    return result;
}
...
void adam_reference_update(const AdamUpdateRequest& request) {
    for (const auto& tensor : request.tensors) {
        ...
        for (std::size_t index = 0; index < value.size(); ++index) {
            ...
            const double update = static_cast<double>(request.learning_rate) *
                                  corrected_first /
                                  (std::sqrt(corrected_second) +
                                   static_cast<double>(request.epsilon));

            next_first[index] = checked_float(first_value, "first moment");
            next_second[index] = checked_float(second_value, "second moment");
            next_value[index] = checked_float(
                static_cast<double>(value[index]) - update, "parameter value");
        }
    }
}
