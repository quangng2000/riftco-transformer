...
std::vector<std::size_t> make_strides(
    ...
    for (std::size_t index = shape.size(); index > 1; --index) {
        ...
        strides[current] =
            strides[current + 1] * shape[current + 1];
    }
    return strides;
}
...
void nn_reference_gelu_forward(
    const GeluForwardRequest& request
) {
    constexpr float one_over_sqrt_two =
    ...
    for (std::size_t index = 0;
        ...
        output[index] =
            0.5F * value *
            (1.0F + std::erf(value * one_over_sqrt_two));
    }
}

void nn_reference_gelu_backward(
    const GeluBackwardRequest& request
) {
    constexpr float one_over_sqrt_two =
        0.70710678118654752440F;
    constexpr float one_over_sqrt_two_pi =
    ...
    for (std::size_t index = 0;
        ...
        const float derivative =
            0.5F *
                (1.0F +
                 std::erf(value * one_over_sqrt_two)) +
            value * one_over_sqrt_two_pi *
                std::exp(-0.5F * value * value);
...
void nn_reference_reduce(const ReductionRequest& request) {
    ...
    for (std::size_t outer = 0;
        ...
        for (std::size_t inner = 0;
            ...
            float total = 0.0F;
            const auto base =
                outer * dimensions.width * dimensions.inner +
                inner;
            for (std::size_t item = 0;
                 item < dimensions.width;
                 ++item) {
                total += input[
                    base + item * dimensions.inner
                ];
            }
...
void nn_reference_permute(const PermuteRequest& request) {
    ...
    for (std::size_t output_index = 0;
        ...
        for (std::size_t output_dimension = 0;
            ...
            input_index +=
                coordinate *
                input_strides[request.axes[output_dimension]];
        }
    ...
    for (std::size_t output_index = 0;
        ...
        for (std::size_t output_dimension = 0;
            ...
            const auto input_coordinate =
                request.input_shape[input_dimension] == 1
                    ? 0
                    : coordinate;
            input_index +=
                input_coordinate * input_strides[input_dimension];
        }
        output[output_index] = input[input_index];
    }
}

void nn_reference_sum_to_shape(
    ...
    for (std::size_t input_index = 0;
        ...
        for (std::size_t input_dimension = 0;
            ...
            const auto output_coordinate =
                request.output_shape[output_dimension] == 1
                    ? 0
                    : coordinate;
            output_index +=
                output_coordinate *
                output_strides[output_dimension];
        }
        output[output_index] += input[input_index];
    }
}
    ...
    for (std::size_t outer = 0;
        ...
        for (std::size_t inner = 0;
            ...
            for (std::size_t item = 0;
                 ...
                 ++item) {
                const float value =
                    input[base + item * dimensions.inner];
                if (std::isnan(value) ||
                    value ==
                        std::numeric_limits<float>::infinity()) {
                    throw std::domain_error(
            ...
            for (std::size_t item = 0;
                ...
                const double exponential = std::exp(
                    static_cast<double>(input[index] - maximum)
                );
                probabilities[index] =
...
void nn_reference_layer_norm_forward(
    ...
    auto inverse_standard_deviation =
    ...
    for (std::size_t row = 0; row < request.rows; ++row) {
        ...
        for (std::size_t column = 0;
            ...
            scale_gradient[column] +=
                upstream[base + column] * normalized;
            bias_gradient[column] += upstream[base + column];
        }

        const float inverse_width =
            1.0F / static_cast<float>(request.width);
        for (std::size_t column = 0;
             column < request.width;
             ++column) {
            const float normalized =
            ...
            input_gradient[base + column] =
                ...
                (
                    scaled_upstream -
                    sum_scaled_upstream * inverse_width -
                    normalized *
                        sum_scaled_upstream_times_normalized *
                        inverse_width
                );
    ...
    for (std::size_t position = 0;
        ...
        for (std::size_t item = 0;
            ...
            exponential_sum += std::exp(
                static_cast<double>(
                    logits[row_offset + item] - maximum
                )
            );
        }

        const auto target =
            static_cast<std::size_t>(request.targets[position]);
        const float target_logit =
            logits[row_offset + target];
