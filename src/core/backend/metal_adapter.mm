...
constexpr const char* kAdamKernelSource = R"METAL(
#include <metal_stdlib>
using namespace metal;
...
public:
    ...
    void materialized_causal_attention_forward(
        ...
        metal_nn_materialized_causal_attention_forward(request);
    }

    void materialized_causal_attention_context_backward(
        const MaterializedCausalAttentionContextBackwardRequest& request
    ) const override {
        metal_nn_materialized_causal_attention_context_backward(request);
    }
